...
class PngConvertActivity : AppCompatActivity() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                withContext(Dispatchers.Main) {
                    binding.tvResult.text = "共找到 ${nonJpgPngUris.size} 张非 JPG/PNG 图片，将开始转换为 PNG..."
                }
                ...
                nonJpgPngUris.forEachIndexed { index, uri ->
                    ...
                    withContext(Dispatchers.Main) {
                        log("(${index + 1}/${nonJpgPngUris.size}) ${if (ok) "✅" else "❌"} 转换: $uri")
                    }
                }
    ...
    private fun convertSingleToPng(srcUri: Uri): Boolean {
        ...
        runOnUiThread {
            log("PNG: 解码 ${tDecodeMs}ms，写PNG ${tEncodeMs}ms，总 ${tTotalMs}ms")
        }
        return result
    }

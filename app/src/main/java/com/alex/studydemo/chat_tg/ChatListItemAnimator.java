package androidx.recyclerview.tgwidget;
...
import com.im.sdk.view.animation.tg.ThanosEffect;
import com.tmmtmm.im.tgchat.cell.TgChatActionCell;
import com.tmmtmm.im.tgchat.cell.TgMessageGroupInfo;
import com.tmmtmm.im.tgchat.cell.TgPhotoImageStub;
import com.tmmtmm.im.tgchat.cell.TgStyleMessageCell;
import com.tmmtmm.im.tgchat.cell.TgTransitionParams;
import com.tmmtmm.im.tgchat.ui.TgChatMessageActivity;
import com.tmmtmm.im.tgchat.util.TgCubicBezierInterpolator;
import com.tmmtmm.im.tgchat.widget.RecyclerListView;
import com.tmmtmm.im.tgchat.widget.TgChatGreetingView;
...
public class ChatListItemAnimator extends DefaultItemAnimator {
    ...
    public static final Interpolator DEFAULT_INTERPOLATOR = new TgCubicBezierInterpolator(0.19919472913616398, 0.010644531250000006, 0.27920937042459737, 0.91025390625);

    @Nullable
    private final TgChatMessageActivity activity;
    private final RecyclerListView recyclerListView;

    private HashMap<Integer, TgMessageGroupInfo> willRemovedGroup = new HashMap<>();
    private ArrayList<TgMessageGroupInfo> willChangedGroups = new ArrayList<>();
    ...
    private TgChatGreetingView chatGreetingsView;

    private boolean reversePositions;
//    private final Theme.ResourcesProvider resourcesProvider;

    //    public ChatListItemAnimator(TgChatMessageActivity activity, RecyclerListView listView, Theme.ResourcesProvider resourcesProvider) {
    public ChatListItemAnimator(TgChatMessageActivity activity, RecyclerListView listView) {
...
//        final boolean supportsThanos = getThanosEffectContainer != null && supportsThanosEffectContainer != null && supportsThanosEffectContainer.run();
        final boolean supportsThanos = false;
        if (supportsThanos) {
            ...
            for (int i = 0; i < mPendingRemovals.size(); ++i) {
                ...
                if (toBeSnapped.contains(holder) && holder.itemView instanceof TgStyleMessageCell && ((TgStyleMessageCell) holder.itemView).getCurrentMessagesGroup() != null) {
                    TgStyleMessageCell cell = (TgStyleMessageCell) holder.itemView;
                    long groupId = cell.getMessageGroupId();
                    if (groupId != 0) {
                        if (groupsToRemoveWithThanos == null) {
                            groupsToRemoveWithThanos = new LongSparseArray<>();
                        }
                        ArrayList<RecyclerView.ViewHolder> holders = groupsToRemoveWithThanos.get(groupId);
                        if (holders == null) {
                            groupsToRemoveWithThanos.put(groupId, holders = new ArrayList<>());
                        }
            ...
            if (groupsToRemoveWithThanos != null) {
                for (int i = 0; i < groupsToRemoveWithThanos.size(); ++i) {
                    ...
                    if (firstHolder.itemView instanceof TgStyleMessageCell) {
                        TgMessageGroupInfo group = ((TgStyleMessageCell) firstHolder.itemView).getCurrentMessagesGroup();
                        if (group != null) {
                            wholeGroup = group.getMessageCount() <= holders.size();
                        }
                    }
                    if (!wholeGroup) {
    ...
    public boolean animateAdd(RecyclerView.ViewHolder holder) {
        ...
        if (!shouldAnimateEnterFromBottom) {
            ...
            if (holder.itemView instanceof TgStyleMessageCell) {
                ((TgStyleMessageCell) holder.itemView).getTransitionParams().messageEntering = true;
            }
        }
    ...
    public void animateAddImpl(final RecyclerView.ViewHolder holder, int addedItemsHeight) {
        ...
        TgStyleMessageCell chatMessageCell = holder.itemView instanceof TgStyleMessageCell ? (TgStyleMessageCell) holder.itemView : null;
        if (!(chatMessageCell != null && chatMessageCell.getTransitionParams().ignoreAlpha)) {
            holder.itemView.setAlpha(1);
        }
        if (activity != null && chatMessageCell != null && activity.getAnimatingMessageObjects().contains(chatMessageCell.getMessageObject())) {
            activity.getAnimatingMessageObjects().remove(chatMessageCell.getMessageObject());
...
//                    }
                    ...
                    // TG 的「输入框→气泡」飞入动画，依赖 SharedConfig / TextMessageEnterTransition，当前无输入框故注释；若后续接入再恢复
        ...
        animation.translationY(0).setDuration(getMoveDuration())
                ...
                .setListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationCancel(Animator animator) {
                        ...
                        if (view instanceof TgStyleMessageCell) {
                            ((TgStyleMessageCell) view).getTransitionParams().messageEntering = false;
                        } else if (view instanceof TgStyleMessageCell) {
                            ((TgStyleMessageCell) view).getTransitionParams().messageEntering = false;
                        }
                    }
...
//        }
        ...
        if (rez) {
            if (info != null) {
                ...
                if (holder.itemView instanceof TgStyleMessageCell) {
                    TgStyleMessageCell chatMessageCell = (TgStyleMessageCell) holder.itemView;
                    if (deltaX != 0) {
    ...
    public boolean animateMove(RecyclerView.ViewHolder holder, ItemHolderInfo info, int fromX, int fromY, int toX, int toY) {
        final View view = holder.itemView;
        TgStyleMessageCell chatMessageCell = null;
        TgChatActionCell chatActionCell = null;
        if (holder.itemView instanceof TgStyleMessageCell) {
            chatMessageCell = ((TgStyleMessageCell) holder.itemView);
        ...
        } else if (holder.itemView instanceof TgChatActionCell) {
            chatActionCell = ((TgChatActionCell) holder.itemView);
        ...
        if (chatMessageCell != null) {
            TgTransitionParams params = chatMessageCell.getTransitionParams();
            ...
            // TgStyleMessageCell.getCurrentMessagesGroup() 返回 TgMessageGroupInfo，此处仅处理 TG ChatMessageCell 的分组
            TgMessageGroupInfo group = chatMessageCell.getCurrentMessagesGroup();
            ...
            if (info instanceof ItemHolderInfoExtended) {
                TgPhotoImageStub newImage = chatMessageCell.getPhotoImage();
                ItemHolderInfoExtended infoExtended = ((ItemHolderInfoExtended) info);
                ...
                if (moveInfo.animateImage) {
                    ...
                    // 起始圆角（from）：animateMove 开始时从 cell 取到的 roundRadius
                    if (params.imageRoundRadius == null) {
                        params.imageRoundRadius = new int[4];
                    }
                    for (int i = 0; i < 4; i++) {
                        params.imageRoundRadius[i] = roundRadius[i];
                    }
                    ...
                    if (params.animateToImageX == infoExtended.imageX && params.animateToImageY == infoExtended.imageY &&
                        ...
                        if (group != null && group.isHasCaption() != group.getTransitionParams().drawCaptionLayout) {
                            TgTransitionParams groupParams = group.getTransitionParams();
                            groupParams.captionEnterProgress = groupParams.drawCaptionLayout ? 1f : 0;
                        }
            ...
            if (group != null) {
                if (willChangedGroups.contains(group)) {
                    ...
                    TgTransitionParams groupTransitionParams = group.getTransitionParams();
                    TgStyleMessageCell lastDrawingCell = null;
                    for (int i = 0; i < recyclerListView.getChildCount(); i++) {
                        View child = recyclerListView.getChildAt(i);

                        if (child instanceof TgStyleMessageCell) {
                            TgStyleMessageCell cell = (TgStyleMessageCell) child;
            ...
            TgMessageGroupInfo removedGroup = willRemovedGroup.get(chatMessageCell.getMessageObject().getId());
            if (removedGroup != null) {
                TgTransitionParams groupTransitionParams = removedGroup.getTransitionParams();
            ...
            TgChatActionCell.TransitionParams params = chatActionCell.getTransitionParams();
...
//        } else
        if (holder.itemView instanceof TgStyleMessageCell) {
            ...
            if (moveInfoExtended.animateImage) {
                ...
                float captionEnterFrom = chatMessageCell.getCurrentMessagesGroup() == null ? params.captionEnterProgress : chatMessageCell.getCurrentMessagesGroup().getTransitionParams().captionEnterProgress;
                float captionEnterTo = chatMessageCell.getCurrentMessagesGroup() == null ? (chatMessageCell.hasCaptionLayout() ? 1 : 0) : (chatMessageCell.getCurrentMessagesGroup().isHasCaption() ? 1 : 0);
                ...
                valueAnimator.addUpdateListener(animation -> {
                    ...
                    if (animateCaption) {
                        ...
                        if (chatMessageCell.getCurrentMessagesGroup() != null) {
                            chatMessageCell.getCurrentMessagesGroup().getTransitionParams().captionEnterProgress = captionP;
                        }
                    }
            ...
            if (moveInfoExtended.animateChangeGroupBackground) {
                ...
                float captionEnterFrom = group.getTransitionParams().captionEnterProgress;
                float captionEnterTo = group.isHasCaption() ? 1 : 0;

                boolean animateCaption = captionEnterFrom != captionEnterTo;
                valueAnimator.addUpdateListener(animation -> {
                    float v = (float) animation.getAnimatedValue();
                    groupTransitionParams.offsetTop = (int) (moveInfoExtended.groupOffsetTop * v);
                    groupTransitionParams.offsetBottom = (int) (moveInfoExtended.groupOffsetBottom * v);
                    groupTransitionParams.offsetLeft = (int) (moveInfoExtended.groupOffsetLeft * v);
                    groupTransitionParams.offsetRight = (int) (moveInfoExtended.groupOffsetRight * v);
                    if (animateCaption) {
            ...
            TgChatActionCell chatActionCell = (TgChatActionCell) holder.itemView;
            TgChatActionCell.TransitionParams params = chatActionCell.getTransitionParams();
        ...
        if (withThanos) {
            animatorSet.setInterpolator(TgCubicBezierInterpolator.Companion.getDEFAULT());
        ...
        animatorSet.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animator) {
                ...
                if (holder.itemView instanceof TgStyleMessageCell) {
                    ...
                    TgMessageGroupInfo group = cell.getCurrentMessagesGroup();
                    if (group != null) {
                        group.getTransitionParams().reset();
                    }
                }
    ...
    public boolean animateChange(RecyclerView.ViewHolder oldHolder, RecyclerView.ViewHolder newHolder, ItemHolderInfo info,
        ...
        if (oldHolder.itemView instanceof TgStyleMessageCell) {
            prevTranslationX = ((TgStyleMessageCell) oldHolder.itemView).getAnimationOffsetX();
        } else {
        ...
        if (oldHolder.itemView instanceof TgStyleMessageCell) {
            ((TgStyleMessageCell) oldHolder.itemView).setAnimationOffsetX(prevTranslationX);
        } else {
        ...
        if (newHolder != null) {
            ...
            if (newHolder.itemView instanceof TgStyleMessageCell) {
                ((TgStyleMessageCell) newHolder.itemView).setAnimationOffsetX(-deltaX);
            } else {
        ...
        if (view != null) {
            ...
            oldViewAnim.alpha(0).setListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animator) {
                    ...
                    view.setScaleX(1f);
                    if (view instanceof TgStyleMessageCell) {
                        ((TgStyleMessageCell) view).setAnimationOffsetX(0);
                    } else {
            ...
            newViewAnimation.translationX(0).translationY(0).setDuration(getChangeDuration())
                    .alpha(1).setListener(new AnimatorListenerAdapter() {
                        ...
                        public void onAnimationEnd(Animator animator) {
                            ...
                            newView.setScaleX(1f);
                            if (newView instanceof TgStyleMessageCell) {
                                ((TgStyleMessageCell) newView).setAnimationOffsetX(0);
                            } else {
    ...
    public ItemHolderInfo recordPreLayoutInformation(@NonNull RecyclerView.State state, @NonNull RecyclerView.ViewHolder viewHolder, int changeFlags, @NonNull List<Object> payloads) {
        ...
        if (viewHolder.itemView instanceof TgStyleMessageCell) {
            TgStyleMessageCell chatMessageCell = (TgStyleMessageCell) viewHolder.itemView;
...
//        } else
        if (view instanceof TgStyleMessageCell) {
            ((TgStyleMessageCell) view).getTransitionParams().resetAnimation();
            ((TgStyleMessageCell) view).setAnimationOffsetX(0f);
        } else if (view instanceof TgChatActionCell) {
            ((TgChatActionCell) view).getTransitionParams().resetAnimation();
        } else {
...
//        }
        for (TgMessageGroupInfo groupedMessages : willChangedGroups) {
            groupedMessages.getTransitionParams().isNewGroup = false;
        }
...
//            }
     ...
     * Tg：分组即将变为单条（如相册删剩一条）
     */
    public void groupWillTransformToSingleMessage(int firstMessageId, TgMessageGroupInfo group) {
        if (group != null) {
            willRemovedGroup.put(firstMessageId, group);
        }
    }

    /**
     * Tg：分组即将变化（增删消息等），记录 bounds 并加入 willChangedGroups
     */
    public void groupWillChanged(TgMessageGroupInfo groupedMessages) {
        if (groupedMessages == null) {
            return;
        }
        TgTransitionParams tp = groupedMessages.getTransitionParams();
        if (groupedMessages.getMessageCount() == 0) {
            tp.drawBackgroundForDeletedItems = true;
        } else {
            if (tp.top == 0 && tp.bottom == 0 && tp.left == 0 && tp.right == 0) {
                int n = recyclerListView.getChildCount();
                for (int i = 0; i < n; i++) {
                    View child = recyclerListView.getChildAt(i);
                    if (child instanceof TgStyleMessageCell) {
                        TgStyleMessageCell cell = (TgStyleMessageCell) child;
                        if (cell.getCurrentMessagesGroup() == groupedMessages && cell.getTransitionParams().wasDraw) {
                            tp.top = cell.getTop() + cell.getPaddingTop() + cell.getBackgroundDrawableTop();
                            tp.bottom = cell.getTop() + cell.getPaddingTop() + cell.getBackgroundDrawableBottom();
                            tp.left = cell.getLeft() + cell.getBackgroundDrawableLeft();
                            tp.right = cell.getLeft() + cell.getBackgroundDrawableRight();
                            tp.drawCaptionLayout = cell.hasCaptionLayout();
...
//        }
        ...
        if (view instanceof TgStyleMessageCell) {
            TgStyleMessageCell cell = (TgStyleMessageCell) view;
        ...
        if (view instanceof TgStyleMessageCell) {
            if (holder == greetingsSticker) {
                ...
                recyclerListView.setClipChildren(false);
                TgStyleMessageCell messageCell = (TgStyleMessageCell) view;
                ...
                TgMessageGroupInfo groupedMessages = ((TgStyleMessageCell) view).getCurrentMessagesGroup();
                if (groupedMessages != null) {
                    Long groupDelay = groupIdToEnterDelay.get(groupedMessages.getGroupId());
                    if (groupDelay == null) {
                        groupIdToEnterDelay.put(groupedMessages.getGroupId(), currentDelay);
                    } else {
                        currentDelay = groupDelay;
                    }
                }
                if (groupedMessages != null && groupedMessages.getTransitionParams().backgroundChangeBounds) {
                ...
                if (groupedMessages != null) {
                    ...
                    if (groupedMessages.getTransitionParams().backgroundChangeBounds) {
                        animatorSet.setStartDelay(140);
                    }
                }
            }
...
//        });
    ...
    public void onGreetingStickerTransition(RecyclerView.ViewHolder holder, TgChatGreetingView greetingsViewContainer) {
...
//        if (viewHolder.itemView instanceof TgStyleMessageCell) {
//            TgStyleMessageCell.TgMessageObjectStub msg = ((TgStyleMessageCell) viewHolder.itemView).getMessageObject();
//            if (msg != null) {
